...
const dialogsReducer = (state: MessagePageType = initialState, action: DialogsReducerActionType): MessagePageType => {
    switch (action.type) {
        case ADD_MESSAGE:{
            let newMessage: MessageType = {id: v1(), message: action.payload.newMessage}
...
export const addMessageAC = (newMessage: string) =>
    ({type: ADD_MESSAGE, payload: {newMessage}}) as const
export const removeMessageAC = (messageForRemoveId: string) =>
    ({type: REMOVE_MESSAGE, payload: {messageForRemoveId}} as const)
