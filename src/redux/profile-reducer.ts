...
const profileReducer = (state: ProfilePageType = initialState, action: ProfileReducerActionType): ProfilePageType => {
    ...
    switch (action.type) {
        case ADD_POST: {
            ...
            let newPost: PostType = {id: v1(), message: action.payload.newPost, likeCount: 0}
...
export type AddPostActionType =  ReturnType<typeof addNewPostAC>//можно делать так, чтобы не дублировать
...
export const addNewPostAC =(newPost: string)=> ({type: ADD_POST, payload: {newPost}}) as const
